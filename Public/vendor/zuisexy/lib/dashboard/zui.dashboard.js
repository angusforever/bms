    ...
    Dashboard.prototype.init = function() {
        ...
        if(options.data) {
            ...
            $.each(options.data, function (idx, config) {
                ...
                var atp_defaultheight = Dashboard.DEFAULTS.height;
                var atp_height = config.colHeight * atp_defaultheight + (config.colHeight - 1) * 20;
                var $panel = $('<div class="panel" data-id="' + (config.id || $.zui.uuid()) + '" style="height: ' + atp_height + 'px;" />', config.panelAttrs);
