...
<script>
    $(function () {
        $('#atpbiztable').bootstrapTable({
            url: '/bms/index.php/Admin/BookType/getData',         //请求后台的URL（*）
            method: 'post',                      //请求方式（*）
...
//            height: 600,                        //行高，如果没有设置height属性，表格自动根据记录条数觉得表格高度
            uniqueId: "tp_atpid",                     //每一行的唯一标识，一般为主键列
...
//            cardView: true,                    //是否显示详细视图
            ...
            columns: [
                [
                    ...
                    {field: 'tp_name', title: '名称', sortable: true},
                    {field: 'tp_atpid', title: '操作', sortable: false,width:150,
                        formatter: function (value, row, index) {
                            var inp = "'"+  value +"'";
                    ...
                    {field: 'tp_atpcreatedatetime', title: '创建时间', sortable: true, visible:false},
                    {field: 'tp_atpcreateuser', title: '创建人', sortable: true, visible:false},
                    {field: 'tp_atplastmodifydatetime', title: '最后修改时间', sortable: true, visible:false},
                    {field: 'tp_atplastmodifyuser', title: '最后修改人', sortable: true, visible:false},
                ]
            ],
            onDblClickRow: function (row) {
                $("#sys_dlg").load('/bms/index.php/Admin/BookType/edit?id=' + row['tp_atpid'], function() {
...
//                console.log(name+order);
            ...
            onPostBody: function(){
                window.parent.setHeight($('body').height() + 50, 1);
            }
        });
    });
    ...
    $('#sys_add').on('click',function(){
        $("#sys_dlg").load('/bms/index.php/Admin/BookType/add', function() {
//            $('#sys_dlg_submit').on('click', submitdata);
            $("#sys_dlg").modal({backdrop: false});
        });
    });
...
//            $.ajax({
//                url: '/bms/index.php/Admin/BookType/submit',
//                type: 'POST',
//                data:  formData,
...
//    }
    ...
    $('#sys_update').on('click',function() {
        ...
        if(tablerow.length!=1)
        {
            ...
            $("#sys_dlg").load('/bms/index.php/Admin/BookType/edit?id=' + tablerow[0]['tp_atpid'], function() {
//                $('#sys_dlg_submit').on('click',submitdata);
                $("#sys_dlg").modal({backdrop: false});
            });
        }
    ...
    {
        $("#sys_dlg").load('/bms/index.php/Admin/BookType/edit?id=' + id, function() {
//            $('#sys_dlg_submit').on('click',submitdata);
            $("#sys_dlg").modal({backdrop: false});
        });
    }
    ...
    function delInRow(id)
    {
        if (confirm('确认删除数据?')) {
            var ids = [];
            ids.push(id);
            $.post('/bms/index.php/Admin/BookType/del', {ids: ids.join(',')}, function (rep, status) {
                if ('' == rep) {
                    $('#atpbiztable').bootstrapTable('refresh')
                }
                else {
